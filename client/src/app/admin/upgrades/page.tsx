...
import { Loader2, CheckSquare, Check, X, Clock, User, CreditCard, Search } from 'lucide-react';
...
export default function AdminUpgradesPage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'pending':
                return <span className="px-2 py-1 bg-yellow-500/10 text-yellow-500 text-xs font-bold rounded-full flex items-center gap-1"><Clock className="w-3 h-3" /> Chờ duyệt</span>;
            case 'approved':
                return <span className="px-2 py-1 bg-green-500/10 text-green-500 text-xs font-bold rounded-full flex items-center gap-1"><Check className="w-3 h-3" /> Đã duyệt</span>;
            case 'rejected':
                return <span className="px-2 py-1 bg-red-500/10 text-red-500 text-xs font-bold rounded-full flex items-center gap-1"><X className="w-3 h-3" /> Từ chối</span>;
            default:
                                    ...
                                    <tr key={item._id} className="hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <CreditCard className="w-3 h-3 text-gold-gradient" />
