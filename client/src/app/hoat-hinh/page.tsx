    ...
    const fetchMovies = async () => {
        ...
        try {
            const res = await fetch(
                `${API_URL}/api/movies?type=hoathinh&page=${page}&limit=24`,
                { credentials: 'include' }
            );
                    ...
                    <>
                        ...
                        {totalPages > 1 && (
                            <div className="flex justify-center gap-2 mt-8">
                                {page > 1 && (
                                    <a href={`/hoat-hinh?page=${page - 1}`} className="px-4 py-2 bg-white/10 hover:bg-primary hover:text-black rounded transition-colors">
                                        Trang trước
                                    </a>
                                )}
